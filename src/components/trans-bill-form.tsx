...
import { CalendarIcon, PlusCircle, Trash2, FileText, Truck, Save, XCircle, FileUp, Hash, Percent, DollarSign, BookOpen, Link as LinkIcon, NotebookText, Briefcase, Anchor, Ship, Box, Layers, Building } from "lucide-react";
...
export function TransBillForm({
  ...
  useEffect(() => {
    if (isEditing && initialData) {
      form.reset({
        ...
        jobDate: initialData.jobDate ? new Date(initialData.jobDate) : undefined,
      });
    } else {
  ...
  const { subTotal, cgstAmount, sgstAmount, totalTax, totalAfterTax, totalPayable } = useMemo(() => {
    ...
    // Note: Assuming symmetric GST. More complex logic might be needed if GST rates differ per item.
    // For now, we take the form-level rates.
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: 'billDocumentUri' | 'lrDocumentUri') => {
    ...
    <Card className="w-full max-w-4xl mx-auto shadow-xl mb-8">
      <CardHeader>
        <CardTitle className="font-headline text-2xl flex items-center gap-2">
          ...
          {isEditing ? "Edit Transport Bill" : "Add New Transport Bill"}
        </CardTitle>
        <CardDescription>Record the bill received from the transporter. Start by linking it to an Export Document.</CardDescription>
      </CardHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <FormField control={form.control} name="lrDocumentUri" render={({ field }) => (<FormItem><FormLabel className="flex items-center gap-2"><FileUp/>Upload LR PDF/Image</FormLabel><FormControl><Input type="file" onChange={(e) => handleFileChange(e, 'lrDocumentUri')} accept=".pdf,.jpg,.jpeg,.png" /></FormControl><FormMessage/></FormItem>)} />
                 </div>
              </CardContent>
            </Card>
          ...
          <CardFooter className="flex justify-end gap-2">
            ...
            <Button type="submit"><Save className="mr-2 h-4 w-4"/> {isEditing ? "Save Changes" : "Save Bill"}</Button>
          </CardFooter>
        </form>
      </Form>
    </Card>
