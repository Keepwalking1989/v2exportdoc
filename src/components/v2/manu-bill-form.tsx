...
export function ManuBillFormV2({
  initialData,
  isEditing,
  onSave,
  onCancelEdit,
  ...
  function onSubmit(values: ManuBillFormValues) {
    const finalBill: ManuBill = {
      id: isEditing ? initialData!.id : '', // Let DB handle ID for new
      ...values,
      items: (values.items || []).map(item => {
        const taxableAmount = (item.quantity * item.rate) * (1 - (item.discountPercentage / 100));
        return { ...item, id: item.id || '', taxableAmount };
      }),
      subTotal,
      finalSubTotal,
      centralTaxAmount,
  ...
  const exportDocOptions: ComboboxOption[] = allExportDocuments.map(doc => ({
    ...
    label: `ED-${doc.id.toString().slice(-6)} (${doc.exportInvoiceNumber})`
  }));
  
  const manufacturerOptions: ComboboxOption[] = allManufacturers.map(m => ({
    value: m.id.toString(),
    label: m.companyName,
  }));
    ...
    <Card className="w-full max-w-4xl mx-auto shadow-xl mb-8">
      <CardHeader>
        <CardTitle className="font-headline text-2xl flex items-center gap-2">
          ...
          {isEditing ? "Edit Manufacturer Bill (DB)" : "Add New Manufacturer Bill (DB)"}
        </CardTitle>
