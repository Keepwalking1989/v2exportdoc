...
export function TransactionFormV2({
  ...
  useEffect(() => {
    ...
    // A more advanced implementation would be to check for existing payments linked to these bills.
    // For now, we assume all bills for the party are potentially payable.
  ...
  const partyOptions = useMemo((): ComboboxOption[] => {
    switch (partyType) {
      case 'client': return allClients.map(c => ({ value: c.id.toString(), label: c.companyName }));
      case 'manufacturer': return allManufacturers.map(m => ({ value: m.id.toString(), label: m.companyName }));
      case 'transporter': return allTransporters.map(t => ({ value: t.id.toString(), label: t.companyName }));
      case 'supplier': return allSuppliers.map(s => ({ value: s.id.toString(), label: s.companyName }));
      case 'pallet': return allPallets.map(p => ({ value: p.id.toString(), label: p.companyName }));
      default: return [];
    }
  ...
  const exportDocOptions = useMemo((): ComboboxOption[] => 
    allExportDocuments.map(d => ({ value: d.id.toString(), label: d.exportInvoiceNumber }))
  , [allExportDocuments]);
    ...
    <Card className="w-full max-w-2xl mx-auto shadow-xl mb-8">
      <CardHeader>
        <CardTitle className="font-headline text-2xl flex items-center gap-2">
          ...
          {isEditing ? "Edit Transaction (DB)" : "New Transaction (DB)"}
        </CardTitle>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-6">
            <FormField control={form.control} name="type" render={({ field }) => ( <FormItem className="space-y-3"><FormLabel>Transaction Type *</FormLabel><FormControl><RadioGroup onValueChange={field.onChange} value={field.value} className="flex space-x-4"><FormItem className="flex items-center space-x-3 space-y-0"><FormControl><RadioGroupItem value="debit" /></FormControl><FormLabel className="font-normal">Debit (Payment Received)</FormLabel></FormItem><FormItem className="flex items-center space-x-3 space-y-0"><FormControl><RadioGroupItem value="credit" /></FormControl><FormLabel className="font-normal">Credit (Payment Made)</FormLabel></FormItem></RadioGroup></FormControl><FormMessage /></FormItem>)}/>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <FormField control={form.control} name="partyType" render={({ field }) => ( <FormItem><FormLabel>Party Type *</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select a party type" /></SelectTrigger></FormControl><SelectContent>{transactionType === 'debit' ? (<><SelectItem value="client"><User className="inline-block mr-2 h-4 w-4"/>Client</SelectItem><SelectItem value="gst"><Landmark className="inline-block mr-2 h-4 w-4"/>Government (GST)</SelectItem><SelectItem value="duty_drawback"><Landmark className="inline-block mr-2 h-4 w-4"/>Government (Duty Drawback)</SelectItem><SelectItem value="road_tp"><Landmark className="inline-block mr-2 h-4 w-4"/>Government (Road TP)</SelectItem></>) : (<><SelectItem value="manufacturer"><Building2 className="inline-block mr-2 h-4 w-4"/>Manufacturer</SelectItem><SelectItem value="transporter"><Truck className="inline-block mr-2 h-4 w-4"/>Transporter</SelectItem><SelectItem value="supplier"><Package className="inline-block mr-2 h-4 w-4"/>Supplier</SelectItem><SelectItem value="pallet"><Palette className="inline-block mr-2 h-4 w-4"/>Pallet</SelectItem></>)}</SelectContent></Select><FormMessage /></FormItem>)}/>
              {!isGovernmentParty && ( <FormField control={form.control} name="partyId" render={({ field }) => ( <FormItem><FormLabel>Party *</FormLabel><Combobox options={partyOptions} value={field.value} onChange={field.onChange} placeholder="Select a party..." disabled={partyOptions.length === 0} emptySearchMessage="No parties of this type found."/><FormMessage /></FormItem>)}/>)}
              {isGovPartyWithDocLink && ( <FormField control={form.control} name="exportDocumentId" render={({ field }) => ( <FormItem><FormLabel className="flex items-center gap-2"><LinkIcon className="h-4 w-4 text-muted-foreground"/>Link to Export Document</FormLabel><Combobox options={exportDocOptions} value={field.value} onChange={field.onChange} placeholder="Select an export document..." disabled={exportDocOptions.length === 0} emptySearchMessage="No export documents found."/><FormMessage /></FormItem>)}/> )}
            </div>
            
            {transactionType === 'credit' && isPayableParty && unpaidBills.length > 0 && (
                <Card><CardHeader><CardTitle>Select Invoices to Pay</CardTitle><CardDescription>Select one or more invoices. The total amount will be suggested automatically, but can be edited for partial payments.</CardDescription></CardHeader><CardContent><FormField control={form.control} name="relatedInvoices" render={({ field }) => ( <FormItem><ScrollArea className="h-48 w-full rounded-md border p-4">{unpaidBills.map((bill) => ( <FormField key={bill.id} control={form.control} name="relatedInvoices" render={({ field: RHFfield }) => { return ( <FormItem key={bill.id} className="flex flex-row items-start space-x-3 space-y-0 mb-2"><FormControl><Checkbox checked={RHFfield.value?.some(item => item.id === bill.id)} onCheckedChange={(checked) => { return checked ? RHFfield.onChange([...(RHFfield.value || []), { type: bill.billType, id: bill.id }]) : RHFfield.onChange(RHFfield.value?.filter((value) => value.id !== bill.id))}}/></FormControl><FormLabel className="font-normal w-full flex justify-between"><span>{bill.invoiceNumber} ({format(new Date(bill.invoiceDate), 'dd-MMM-yy')})</span><span className="font-medium">₹ {Number(bill.grandTotal || (bill as TransBill).totalPayable || 0).toFixed(2)}</span></FormLabel></FormItem>)}}/>))}</ScrollArea></FormItem>)}/></CardContent></Card>
            )}
