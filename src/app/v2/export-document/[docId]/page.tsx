...
export default function DocumentDataPageV2() {
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            const docData = await docRes.json();
             const parsedDoc: ExportDocument = {
                ...docData,
            ...
            toast({ variant: "destructive", title: "Loading Error", description: "Could not fetch document details."});
        } finally {
            setIsLoading(false);
        }
    };
  ...
  const onSaveShippingBill = async (data: ShippingBillFormValues) => {
    if (await updateDocumentInDb(data)) {
  ...
  const onSaveBl = async (data: BlFormValues) => {
    if (await updateDocumentInDb(data)) {
  ...
  const onSaveBrc = async (data: BrcFormValues) => {
    if (await updateDocumentInDb(data)) {
  ...
  const ewayBillData = useMemo(() => {
    ...
    return {
      ...
      exportInvoiceDate: format(new Date(document.exportInvoiceDate), 'dd-MM-yyyy'),
  ...
  if (!document) {
    return <div className="flex flex-col min-h-screen bg-background"><Header /><main className="flex-grow container mx-auto px-4 py-8"><Card><CardHeader><CardTitle>Document Not Found</CardTitle></CardHeader><CardContent><p>The requested export document could not be found.</p><Button onClick={() => router.push('/v2/export-document')} className="mt-4"><ArrowLeft className="mr-2 h-4 w-4" /> Go Back to List</Button></CardContent></Card></main></div>;
  }
